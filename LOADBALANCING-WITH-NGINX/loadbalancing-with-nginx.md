...
[nginx-lb running]
Ensure you start all the webserver instances, DB-server instance and NFS server instance.

- Connect to the `nginx-lb` EC2 instance via SSH. Update the `/etc/hosts` filed for local DNS and their local IP addresses. 
...
- To get a valid SSL certificate, you should register a new domain name. I have a registered subdomain from Afraid.dns for test purposes. `laraadeboye.mooo.com`
...
Assuming we want to schedule the cron job for certbot to run on the first day of every month:
